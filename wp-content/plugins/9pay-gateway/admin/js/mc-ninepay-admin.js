...
jQuery( document ).ready(
	function(){
		...
		jQuery( '.ninepay-element-bnpl' ).parents( 'tr' ).addClass( 'ninepay-wrapper-bnpl' );
		...
		jQuery( '.ninepay-percent-installment' ).parents( 'fieldset' ).find( 'legend' ).replaceWith( '<div><p style="margin-right: 30px;">Số tiền thanh toán trả góp tối thiểu</p></div>' );
		jQuery( '.ninepay-percent-installment' ).attr( "min" , "3000000" )
		jQuery( '.ninepay-percent-installment' ).attr("required",true)
	}
);
